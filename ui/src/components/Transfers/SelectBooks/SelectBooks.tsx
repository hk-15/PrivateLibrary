...
export const SelectBooks: React.FC<Props> = ({
  searchTerm,
  getSelectedBooks,
}) => {
        ...
        {books.length === 0 ? (
          ...
          books.map((b) => (
            <tr key={b.id}>
              ...
              <td>{b.title}</td>
              <td>
                {b.authors.length > 1 ? `${b.authors.join(", ")}` : b.authors}
              </td>
              ...
              <td>
                <input
                  ...
                  id={b.id.toString()}
                  name={b.id.toString()}
                  checked={selectedBooks.some((book) => book.id === b.id)}
                  onChange={(e) => {
